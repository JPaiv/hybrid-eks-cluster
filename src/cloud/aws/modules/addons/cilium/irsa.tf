# ---------------------------------------------------------------------------------------------------------------------
# CILIUM OPERATOR IRSA
# AWS-Managed EKS CNI Policy does not have all the permissions the Cilium needs
# ---------------------------------------------------------------------------------------------------------------------
...
resource "aws_iam_role" "this" {
  ...
  tags = {
    "Description" = "IRSA for the cilium-operator"
    "Name"        = local.irsa_name
  }
}

data "aws_iam_policy_document" "trust_relationships" {
  policy_id = "OidcIrsaTrustPolicy"
  version   = "2012-10-17"
  statement {
    effect = "Allow"
...
resource "aws_iam_policy" "permissions" {
  ...
  tags = {
    "Description" = "Service account cilium-operator IRSA Permissions"
    "Name"        = "${aws_iam_role.this.name}-policy"
  }
}
...
data "aws_iam_policy_document" "permissions" {
  policy_id = "CiliumAdditionalPermissions"
  version   = "2012-10-17"
  statement {
    effect = "Allow"
    sid    = "AllowCiliumEC2Networking"
