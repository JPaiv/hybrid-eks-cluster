resource "aws_iam_role" "this" {
  ...
  tags = {
    "Description" = "Authentik server IRSA"
    "Name"        = local.irsa_name
  }
}
...
resource "aws_iam_policy" "this" {
  name   = "${aws_iam_role.this.name}-permissions-policy"
  policy = data.aws_iam_policy_document.permissions.json

  tags = {
    "Description" = "Authentik server ${local.name_label} IRSA Permissions"
    "Name"        = "${aws_iam_role.this.name}-permissions-policy"
  }
}
